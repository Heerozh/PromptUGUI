...
| `PUI-GRID-CHILD-SIZE`         | `size` / `width` / `height` on a direct child of a cell grid — a `<Grid>`, or a `<ScrollList columns=…>` in grid mode. `GridLayoutGroup` gives every child a uniform `cellSize` set on the parent, so the child's own size is silently overridden. Set the cell size on the parent (`cellSize="WxH"`), or move the element out of the grid for a non-uniform size. (`<VStack>` / `<HStack>` and single-column `<ScrollList>` children are unaffected — there a child's size IS the main-axis size; a `flow="false"` child is exempt — out of flow, its size is meaningful again.) | **CLI-only**             |
| `PUI-SCROLL-COLUMNS-DIRECTION` | `columns` and `direction="horizontal"` on the same `<ScrollList>` (either in the base or in any variant). The grid wraps into rows that grow downwards, so it only ever scrolls vertically, and a row-major grid (`rows=`) does not exist in v1 — at runtime the direction wins and the columns are dropped. Remove `direction="horizontal"` for a grid, or `columns` for a single scrolling row. | CLI error + runtime warning |
| `PUI-SCROLL-COLUMNS-CELLSIZE`  | `columns` on a `<ScrollList>` with no `cellSize` (base, any variant, or through `class=`). In grid mode `cellSize` is the only thing that sizes an item — a child's own size is ignored (see `PUI-GRID-CHILD-SIZE`) — so without it every cell silently falls back to uGUI's 100×100. Add `cellSize="WxH"`. `columns="0"` (the documented "no grid" spelling) does not trigger it. | CLI error + runtime warning |
...
| `PUI-RAYCAST-TAG`             | `raycastTarget=` on anything but `<Frame>` / `<Image>` / `<RawImage>` / `<Text>` — layout containers have no Graphic to hit, `<Icon>` / `<Decor>` / `<Progress>` are always click-through, and an interactive control catches the pointer by definition. Seen through `class=` too. | CLI + runtime warning |
| `PUI-RAYCAST-UNDECIDED`       | The outermost drawn surface on a path from the Screen root (a `<Frame>` with a procedural visual attribute, an `<Image>` / `<RawImage>`) carries no `raycastTarget` — a panel that never said whether it blocks what is behind it. Write `true` (a panel) or `false` (a decoration). Inside a `true` node or an interactive control nothing more is asked; under a `false` one the next drawn surface is. Add-block children are judged as outermost. | **CLI-only**             |

...
namespace PromptUGUI.Editor
{
    ...
    /// class only decides which files, how an <c>&lt;Import src&gt;</c> maps to one of them
    /// (<see cref="UiXmlLocator"/>), and where a line goes.
    ///
    ...
    internal static class UIXmlLintMenu
    {
        ...
        {
            var paths = UiXmlLocator.FindProjectUiXml();
            if (paths.Count == 0)
            {
        ...
        {
            var run = new LintRun();
            var resolve = UiXmlLocator.MakeResolver();
            try
            {
        ...
        private static string Read(string path) => File.ReadAllText(UiXmlLocator.Physical(path));
        ...
        /// </summary>
        internal static void Report(LintRun.Finding finding)
        {

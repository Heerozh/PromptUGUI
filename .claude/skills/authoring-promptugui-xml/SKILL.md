...
TAB/TOGGLE    + selectedColor / selectedModulate  (<Btn> has no selected*; applies while the control is the active/isOn one at rest)
...
`selectedColor` / `selectedModulate` are meaningful only on `<Tab>` / `<Toggle>` (apply while the control is the active/`isOn` one at rest); a `<Btn>` has no selected state and ignores them.

...
**Mipmaps (matter for `blur` / `glow` radii past a few texels).** The effects sample the mip level that matches their tap spacing, so a Bilinear atlas with *Generate Mip Maps* on gives a smooth blur at any radius; without a mip chain they sample full resolution and draw ghost copies of thin strokes once the radius is past ~3 texels (the runtime warns once per texture, once those ghosts are far enough apart on screen to form a pattern — 0.35 × radius × canvas scale factor ≥ 2.5 px — so a 2-unit blur on a minified icon is quiet even on a 3x phone, a 3-unit one there warns). `Sync Atlases` does not turn mipmaps on for you — it is a per-atlas trade: a third more memory, and every sprite in the atlas is softened when drawn smaller than 1:1. Keep the atlas padding at Unity's default (the sampling assumes at least 2 texels) and leave *Alpha Is Transparency* on for source PNGs, otherwise the blur's colour darkens at the edges. Point-filtered pixel atlases gain nothing from mipmaps here and should keep their radii small.

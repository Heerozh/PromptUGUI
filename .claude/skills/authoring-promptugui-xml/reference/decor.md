...
| `color` | all | the full gradient grammar: token / hex / CSS name / `/alpha` / `[to right,] A, B[, C[, D]]` / `A 70%,B` 色标 / `A,70%,B` 提示 | `white` | drawn by the SDF shader, so direction, 2–4 stops, positions and hints all apply — in the decoration's **canonical frame** (bracket hugging its top-left, tick pointing down): mirrored instances mirror the ramp too, which is what keeps four corner brackets identical and on one material. On `sprite` it is a plain tint (no gradient) |
| `glow` / `glowColor` | SDF kinds | same as `<Frame>` — `glowColor` takes the full gradient grammar | `0` / follows the whole fill ramp | inflates the drawn quad only, never the layout; an unset `glowColor` is the fill ramp at full alpha |

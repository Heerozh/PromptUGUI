...
- `<Image ... tint="multiply|linear"/>` — Image 混合材质：`multiply`（默认，UI/Default）或 `linear`（Linear Light，128 灰中性）。所有 Image-backed 控件（Image / Icon / Btn / Toggle / Slider / Dropdown / ScrollList / InputField / Progress）通用，`<Text>` 除外；见 [`2026-05-28-image-tint-blend-mode-design.md`](2026-05-28-image-tint-blend-mode-design.md)

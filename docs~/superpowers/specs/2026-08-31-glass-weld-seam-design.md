...
### 8.6 台阶改为逐条轮廓累加（第三次上机反馈后）

§8.5 的向内剖面上机后，交界处出现一条**竖线截断**：台阶沿资源块斜边走到玩家卡右边界（画布 x=166）
就被硬切，并在切口内侧多出一条沿玩家卡边缘的假线。

根因在**求导方式**，不在剖面。§2.1 把逐块厚度折叠成高度场 `h = acc_h / cov` 再解析求 `∇h`，商法则
会把 `∇cov` 带进来 —— 即**下层块自己轮廓上的覆盖率梯度**。向外时它不显形：上层块在下层块轮廓所在
的那片区域覆盖率恒为 1，`lerp` 把下层的变化整个覆盖掉；向内时上层块在自己轮廓附近覆盖率 < 1
（整条斜坡带都是），下层的边就漏了出来。同一处还有第二个毛病：下层材料到头时台阶硬切。

改法：**不再对高度场求导，改成逐条轮廓累加台阶梯度**，每条乘一个「身下有多少材料可踩」的闸门 ——

```
stepGrad += (depth_j · cov − acc_h) · ∇ramp_j        // cov / acc_h 取折叠到 j 之前的值
acc_h = lerp(acc_h, depth_j, c_j);  cov = lerp(cov, 1, c_j)
```

`(depth_j − acc_h/cov)·cov` 恰好化简成 `depth_j·cov − acc_h`，连除法都不用。两处关键：

1. **闸门只做乘法、不参与求导** —— 下层块的轮廓不再画出自己的线；
2. **折叠用的软覆盖 `c_j` 恒取向外那条**（一块材料一直实到自己的轮廓、再往外软收），于是上层块
   轮廓跑出下层材料时，闸门跟着覆盖率在 `seam` 内**淡出**，台阶柔和收尾而不是被硬切。

不变式反而更强、更容易看出来：单块 `cov = 0` ⇒ 项恒为 0；等厚 `depth_j·cov == acc_h` ⇒ 项恒为 0，
两者都是**严格**零而不再依赖商法则的抵消。向外的既有观感不变：在下层材料完整（`cov = 1`）处，
新式子逐字化简回旧的 `∇h = (D_j − D_below)·∇c`。tint 折叠随之统一走向外剖面（负 seam 下 tint 的
分界因此固定在轮廓上；正 seam 逐像素不变）。

客户端离屏实测（玩家卡宽度未动，仍是 166）：旧式在 x≈638 处 48.3 → 37.9 → 11.0 硬跌；新式同一行
36.2 → 32.4 → 23.6 → 16.3 → 12.8 平滑收尾，且台阶继续沿斜边往下走完。EditMode 全绿。

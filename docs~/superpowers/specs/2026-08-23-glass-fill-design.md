...
## 16. 模糊核修正：稀疏 Kawase 把亮点复制成 4×4 点阵（2026-09-02）

### 16.1 现象与根因

星图上一个亮点透过玻璃看是 4×4 共 16 个点的阵列，不是一团光；小星星随地图平移时隐时现。
§13.7 的三次 blit 链每一次都是同一个 4-tap Kawase 核，而 4-tap 核只有在**每个 tap 的 bilinear
足迹够到相邻 tap** 时才是模糊 —— 这与 image-fx spec §14.1 的诊断是同一条规则（tap 间距 > 足迹
→ 比间距细的特征逐 tap 复制）。三处都违反：

| 环节 | 偏移 | tap 间距 | 单 tap 足迹 | 后果 |
|---|---|---|---|---|
| 降采样（4×） | 2 源纹素 | 4 源纹素 | 2 源纹素 | 四个 tap 落在 4×4 块四角，中间两行两列**永远采不到**（源图 25% 覆盖） |
| 轻模糊 | 2 纹素 | 4 纹素 | 0（整数偏移落在纹素中心，退化成点采样） | 纯复制，零模糊 |
| 重模糊 | 3.5 纹素 | 7 纹素 | 2 纹素 | 4 个点各再拆 4 个 = 16 |

面板再把 1/4 分辨率的 B 图 bilinear 放大 4 倍，每个团约 12 px 宽、之间留缝，肉眼即点阵。

### 16.2 定参（预演）

链路每一步都是可分离核，按 GPU bilinear 语义（纹素中心 k+0.5、clamp）做 1D 冲激响应，对
`{0.5, 1.5, 2.5, 3.5, 4.5}` 的偏移序列穷举，指标：屏幕空间剖面从峰值向外的**最大回升**（波纹）、
等效高斯 σ、对高斯的 RMSE。脚本一次性，不入库（与 image-fx §14.9 同一取舍）。

| 链路 | blit | σ（屏幕 px） | 波纹 | RMSE |
|---|---|---|---|---|
| 旧 A：降采样 2 | 1 | 3.25（且偏离块中心的 1 px 点**完全丢失**） | 0 | — |
| 旧 B：+2, 3.5 | 3 | — | **0.93** | 0.27 |
| 教科书 {0.5, 1.5} → A | 3 | 9.5 | **0.29**（中心塌陷） | 0.19 |
| 教科书 {0.5, 1.5, 2.5, 3.5} → B | 5 | 20.0 | 0.034 | 0.064 |
| **新 A：降采样 1 → 0.5** | 2 | 3.5 | 0 | 0.018 |
| **新 B：A → 1.5, 1.5, 1.5, 2.5** | 6 | 16.5 | 0 | 0.019 |

新 B 的 16.5 px 恰是旧链「若核是稠密的」本该达到的强度（√(2² + 3.5²) 个四分之一纹素 ≈ 16 px），
即保持设计手感只修掉稀疏。规则：降采样偏移 **1 个源纹素**（四个 2×2 读数恰好铺满 4×4 块 =
mip level 2 的 box）；同分辨率 pass 一律 **n + 0.5**（tap 落在纹素角上，bilinear 替它做 2×2）。

### 16.3 落点

- `GlassBackdropSystem`：`DownsampleTaps = 1`，`LightBlurTaps = {0.5}`，`HeavyBlurTaps = {1.5, 1.5,
  1.5, 2.5}`，每个 pass 一份材质；RenderGraph 里按序在 scratch / 目标间往返，A 落 `_light`、B 落
  `_heavy`（三张常驻 RT 不变）。成本 3 → 6 次 1/4 分辨率 blit，仍约 1.5 tap / 全分辨率像素。
- 面板 shader、A/B 插值、`frost` 语义、公开 API 不变；glass.md 的成本一节改成六次 blit。
- 不采用 image-fx 的 `tex2Dlod` 圆盘：玻璃是整条底栏级的大面积，25 tap（开 dispersion 再 ×3）
  per fragment 远贵于 blit 链；预滤波留在链里、面板仍两次采样。

### 16.4 测试（Red 先行，`GlassRenderTests`）

- `HeavyBlur_SmearsAPointIntoOneBlob_NotALattice`：黑世界里一个与降采样块对齐的 4×4 白方块，
  `frost=1`、`depth=0`、`lightIntensity=0`、`noise=0`，读穿过方块的一行：从峰值向两侧的最大
  回升 < 2/255，且 10% 峰值以上宽度 > 24 px。修前剖面 `18 18 18 → 7 7 7 → 18 18`，回升 11/255。
- `Capture_SeesEveryPixel_NotJustBlockCorners`：2×2 白方块放在某 4×4 块的中间两行两列，
  `frost=0`，行峰值 > 0.03。修前整行为 0。
- 两条测试把采集相机后移到自身远平面之外并清成黑：harness 默认橙色世界会点亮整行；两台相机
  同在原点时，采集相机会顺带拍到玻璃面板、UI 相机会看到白方块。

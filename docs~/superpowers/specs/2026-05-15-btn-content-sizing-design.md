...
| BCS-D7 | 自由定位 fallback 规则 | `Control.ApplyCommon` else 分支：anchor 该轴不 stretch + 该轴 `HasWidth/HasHeight=false` + `GetNativeSize()` 不为 null → 把 native 填到该轴的 sizeDelta。**Revised 2026-05-27**（原决策：两轴都没写才整体 fallback；见 BCS-D8）。 | 比"作者必须写 size"友好；对 anchor=stretch 轴零影响（那条轴已经被 margin 接管）；目前 GetNativeSize 重写的只有 Icon（已经走 native 路径）和新加的 Btn，行为不回归 |
| BCS-D8 | fallback 是否分轴 | **Revised 2026-05-27**：分轴 fallback。任一轴没写 → 该轴单独用 native 兜底，已写的轴保留作者值。`Control.ApplyLayoutElement` 同步收敛：从原"both-missing 快路径 + explicit-size 慢路径"重构成单一 per-axis 路径。 ~~原决策：只在"两轴都没写"时整体 fallback；任一轴写了 size/width/height 就放弃 native 整体逻辑~~ | 真实 case `<Text height="12">Lv. 45</Text>` 触发 width=0 footgun；原决策的"语义复杂"担忧（width="100" 但 height 不写时取 native.y 还是 0）实际答案明确：取 native.y，符合 CSS `inline-block` 直觉，作者也没主动放弃 height（只是没写）。`size="native"` 关键字保留作显式两轴 native 入口，与分轴 fallback 不冲突（fallback 在 `WithNativeResolved` 之后跑，互斥）。 |

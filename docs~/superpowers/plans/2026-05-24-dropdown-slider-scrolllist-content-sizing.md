...
- [ ] `DropdownContentSizingTests` (5 tests), `SliderContentSizingTests` (6 tests), `ScrollListContentSizingTests` (5 tests) all pass.
- [ ] Existing `DropdownTests` / `SliderTests` / `ScrollListTests` / `BtnContentSizingTests` / `ToggleContentSizingTests` still pass.
- [ ] Full `PromptUGUI.Tests.EditMode` suite green.
- [ ] `dotnet format --verify-no-changes --severity warn` exits 0.
- [ ] SKILL.md mentions `<Dropdown>`、`<Slider>`、`<ScrollList>` in both the LayoutGroup auto-LE list and the free-positioning fallback list, and each table row documents its default size.
- [ ] 7-8 commits on the branch: 3× (red, green) pairs + docs (SKILL.md) + optional lint chore.

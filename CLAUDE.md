...
- Changes to a control / feature that has its own `reference/*.md` → edit **that file**, not (only) the main `SKILL.md`: `<Trigger>` / `<Animation>` → `reference/animations.md`; Btn/Tab/Toggle state visuals (`*Color` / `*Modulate` / `<Show on="state-*">` / `pressedSprite` / `selectedSprite`) → `reference/states.md`; `<TabBar>` / `<Tab>` / `<TabMenu>` → `reference/controls-tabs.md`; `<Collapsible>` / `<Header>` → `reference/controls-collapsible.md`; `<Carousel>` → `reference/controls-carousel.md`; `<Progress>` → `reference/controls-progress.md`; icon-name / SpriteSet discovery → `reference/icons.md`; glass fill (`glass` / `frost` / `depth` / `dispersion` / `lightAngle` / `lightIntensity` / `saturation` / `noise` / `weld`) → `reference/glass.md`；`<Decor>`（角/边装饰：`kind` / `at` / `extent` / `thickness` / `inset` / `offset` / `mirror`）→ `reference/decor.md`. Keep the main-doc primitive-catalog row + stub pointer in sync when attributes are added/removed.
